...
public class Aerolinea
{
    ...
    {
        // TODO implementar
    	List<Tiquete> tiquetesTotales = new ArrayList<>();
    	for (Vuelo vuelos: vuelos) {
    		for (Tiquete tiquetes: vuelos.getTiquetes()) {
    			tiquetesTotales.add(tiquetes);
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
    	...
    	for(Vuelo vuelo:vuelos) {
    		if(vuelo.getAvion().getNombre().equals(nombreAvion) && vuelo.getFecha().equals(fecha) && vuelo.getRuta().getCodigoRuta().equals(codigoRuta)) {
    			throw new Exception("Este vuelo ya existe");
    		}
    		
    		if(vuelo.getFecha().equals(fecha) && vuelo.getAvion().getNombre().equals(nombreAvion) ) {
    			throw new Exception("Este avion esta ocupado");
    		}
    	if(!rutas.containsKey(codigoRuta)) {
        	throw new Exception("No existe esta ruta");
        	}
        if (contador==0) {
       		throw new Exception("No existe este avion");
        	}
    	
    	Vuelo vueloNuevo = new Vuelo(rutas.get(codigoRuta),fecha,avionDisponible);
        		...
        		for (Vuelo vuelo:vuelos) {
        			...
        			if(vuelo.getRuta().getCodigoRuta().equals(codigoRuta)&& vuelo.getFecha().equals(fecha) ) {
        				...
        				for (Tiquete tiquete:tiquetes) {
        					tarifa = tarifa + tiquete.getTarifa();
        				}
        		}}}
        	else if ((9<=mes && mes<=11)||(1<=mes && mes<=5)) {
            		for (Vuelo vuelo:vuelos) {
    ...
    public String consultarSaldoPendienteCliente( String identificadorCliente )
    {
        ...
        for(Tiquete tiquete:getTiquetes()) {
            if (!tiquete.esUsado() && tiquete.getCliente().equals(cliente)) {
            		saldoTotal = saldoTotal + tiquete.getTarifa();
            	}
            }
            
        	return Integer.toString(saldoTotal);
    }
